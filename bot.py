...
async def on_message(message):
    swear = False
    y = '[,. 0123456789 _+\-.\\,!@#$%^&*();\/|<>"]'
    if str(message.author) != "CTAM#8169":
        mes = message.content
        for w in words:
            if re.search(y+w+y,mes,re.IGNORECASE) or mes.startswith(w) or mes.endswith(w):
                mes = re.sub(w,'`****`',mes)
                swear = True
        if swear:
            await client.send_message(message.channel, message.author.mention + " : " + mes)
